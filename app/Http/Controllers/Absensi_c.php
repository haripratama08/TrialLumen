...
class Absensi_c extends Controller
{
    ...
    public function add_absensi(Request $request)
    {
        ...
        if ($jenis_absen == 'absen_masuk') {
            ...
            // print_r($id);
            // echo($id_karyawan);
            $absensi_masuk = Absensi_m::getLastRowIAbsensiMasuk($id_karyawan, $kode_absen);
            // print_r($absensi_masuk);
            // exit();
            ...
            $id = Absensi_m::getId($id_company, $table);
            $data1 = array('id_istmulai' => $id, 'id_absensi_masuk' => Absensi_m::getLastRowIAbsensiMasuk($id_karyawan, $kode_absen)->id_absensi_masuk);
        } else if ($jenis_absen == 'istirahat_selesai') {

            $table = 'istirahat_selesai';
            $id = Absensi_m::getId($id_company, $table);
            $data1 = array('id_istselesai' => $id, 'id_absensi_masuk' => Absensi_m::getLastRowIAbsensiMasuk($id_karyawan, $kode_absen)->id_absensi_masuk);
        ...
        if ($kode_absen == 'TS' && $jenis_absen == 'absen_pulang') {

            $cekAbsenMasuk = DB::table('absensi_masuk as am')
                ->selectRaw('am.tgl_absen')
            ...
            $data2 = array(
                ...
                'latitude' => $lat,
                'longitude' => $long,
                'tgl_absen' => $cekAbsenMasuk->tgl_absen,
                'jam_absen' => $current_date . ' ' . $jam_absen,
        ...
        $id_shift_karyawan = Absensi_m::getId($id_company, 'data_shift_karyawan');
        $dataJadwalShift = array(
            'id_shift_karyawan' => $id_shift_karyawan,
            'tanggal' => $tgl_absensi,
            'id_karyawan' => $id_karyawan,
            'id_master_shift' => $id_master_shift,
            'id_cabang' => $id_cabang,
            'id_departemen' => $id_cabang,
            'user_input' => $id_cabang,
            'id_company' => $id_company
        );


        $data = array_merge($data1, $data2);
        return Absensi_m::addAbsensi($table, $data, $dataJadwalShift);
    }
