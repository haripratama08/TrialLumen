...
use Illuminate\Database\Eloquent\Model;
    ...
    class Patroli_m extends Model{


        public static function pengajuan_patroli($data = array()){
            $insert = DB::table('data_patroli')->insert($data);
            return $insert;
        }
        ...
        public static function getDataPatroli($data = array()){
            ...
            $id_patroli             = $data['id_patroli'];
            $month_year             = $data['month_year'];
            $range_tanggal_mulai    = $data['range_tanggal_mulai'];
            $range_tanggal_selesai  = $data['range_tanggal_selesai'];
            DB::enableQueryLog();
            $data_sos = DB::table('data_patroli as DP')
            ->select('DP.*', 'DK.nama_lengkap')
            ->join('data_karyawan as DK', 'DP.id_karyawan', '=', 'DK.id_karyawan')
            ...
            if($konteks=='dataSaya') $where = [array('DP.id_karyawan', '=', $id_karyawan)];
            ...
            else if($konteks=='dataDetail') $where[] = array('DP.id_patroli', '=', $id_patroli);
            //
            ...
            if($month_year!=null) $data_izin = $data_izin->where(DB::raw("DATE_FORMAT(DP.tgl_input,'%Y-%m')"), $month_year);

            if($range_tanggal_mulai!=null && $range_tanggal_selesai!=null) {
                 $where[] = array('DP.tanggal', '>=', $range_tanggal_mulai);
                 $where[] = array('DP.tanggal', '<=', $range_tanggal_selesai);
            }
            ...
            foreach ($data_sos as $row) {
                ...
                $respon[] = array(
                    'id_patroli'           => $row->id_patroli,
                    'id_karyawan'       => $row->id_karyawan,
                    'tgl_pengajuan'     => DateFormat::format($row->tanggal,'N d M Y'),
                    'nama_lokasi'         => $row->nama_lokasi,
                    'keterangan'         => $row->keterangan,
                    'nama'              => $row->nama_lengkap,
                    'list_image'        => self::getListImage($row->id_patroli),
                );   
            }

            if (count($data_sos)>0) 
                $response = array('success' => true, 'message' => 'data patroli berhasil ditemukan', 'data_patroli' => $respon);
            else 
                $response = array('success'=>false,'message'=> 'data patroli gagal ditemukan');
            return response()->json($response,200);
        }

        public static function getListImage($id){
            $data =  DB::table('file_patroli')->select('file')->where('id_patroli', $id)->get();
